	...
	while not (currentNode in destList):
		...
		for i in G.nodes[currentNode]['buckets'][bucketIndex]:
			...
			if min < 0:
				...
				min = xorResult
				nextHop = i
		thisHopDist = G[bucketOperations.binaryToDecimal(int(nextHop))][currentNode]['weight'] + G.nodes[bucketOperations.binaryToDecimal(int(nextHop))]['nodeLatency'] 
...
def simpleLookupPR(G, src, dest, alpha, useLookupKBR, useLookupDHT, DHTCopiesNumber):
	...
	while alphaCounter < len(currentNodes):
		networkEdges[alphaCounter].append((src, currentNodes[alphaCounter]))
		currentRTT, currentHopCount, currentPathNodes = basicLookupPR(G, currentNodes[alphaCounter], dest, useLookupKBR, useLookupDHT, DHTCopiesNumber, networkEdges[alphaCounter])
		currentRTT += (G[src][currentNodes[alphaCounter]]['weight'] + G.nodes[currentNodes[alphaCounter]]['nodeLatency'])
		currentHopCount += 1
		RTTs_in_EucDist.append(currentRTT)
		hopCounts.append(currentHopCount)
