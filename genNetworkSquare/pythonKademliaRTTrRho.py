...
if parameters.USE_SAVED_GRAPH:
	...
	while nodeGenerationCounter < parameters.NETWORK_SIZE: 
		...
		if parameters.USE_AREA_WITH_HIGH_NODE_LATENCY:
			...
			G.nodes[nodeGenerationCounter]['nodeLatency'] = 0 # random.choice(parameters.NODE_LATENCY_UNIFORM)
			...
			while RLLoopNodeCounter < parameters.NETWORK_SIZE:
				if len(G.nodes[RLLoopNodeCounter]['RLTables'][currentBucketIndex][parameters.REINFORCEMENT_LEARNING_TABLE_DEST_COLUMN_INDEX]) >= parameters.EPOCH_QUERIES_NUMBER:
					...
					if G.nodes[RLLoopNodeCounter]['prevScores'][currentBucketIndex] > 0: # not first epoch
						# set threshold for comparsion
						dynamicCP = parameters.DYNAMIC_CP 
						if currentBucketScore * dynamicCP > G.nodes[RLLoopNodeCounter]['prevScores'][currentBucketIndex] and G.nodes[RLLoopNodeCounter]['bucketChangeItemFlags'][currentBucketIndex] == False: # roll back when new epoch performs badly
						...
						elif currentBucketScore * dynamicCP <= G.nodes[RLLoopNodeCounter]['prevScores'][currentBucketIndex] and G.nodes[RLLoopNodeCounter]['bucketChangeItemFlags'][currentBucketIndex] == False: # ready to change
							G.nodes[RLLoopNodeCounter]['bucketChangeItemFlags'][currentBucketIndex] = True
						elif G.nodes[RLLoopNodeCounter]['bucketChangeItemFlags'][currentBucketIndex]: # change # currentBucketScore * dynamicCP <= G.nodes[RLLoopNodeCounter]['prevScores'][currentBucketIndex] and
							...
							while len(bucketItemCandidate_str) < parameters.NODE_ID_LENGTH:
								...
								if len(bucketItemCandidate_str) == parameters.NODE_ID_LENGTH:
									if RLLoopNodeCounter == observationNode and bucketOperations.binaryToDecimal(int(bucketItemCandidate_str)) in parameters.RHO_ATTACKER:
										currentRTT = parameters.RTT_R1[currentBucketIndex]
									else:
										currentRTT = dist(G.nodes[RLLoopNodeCounter]['pos'], G.nodes[bucketOperations.binaryToDecimal(int(bucketItemCandidate_str))]['pos'])
									# print("CURRENT RTT: " + str(currentRTT) + "BUCKET: " + str(currentBucketIndex))

									reSelectFlag = False
									if currentRTT < parameters.RTT_R1[currentBucketIndex]:
										reSelectFlag = True
									# elif currentRTT < parameters.RTT_R2[currentBucketIndex]:
									# 	randomRange_R2R1 = parameters.RTT_R2[currentBucketIndex] - parameters.RTT_R1[currentBucketIndex]
									# 	positiveRange_R2R1 = currentRTT - parameters.RTT_R1[currentBucketIndex]
									# 	if random.uniform(0, randomRange_R2R1) < positiveRange_R2R1:
									# 		reSelectFlag = True
									# elif currentRTT >= parameters.RTT_R2[currentBucketIndex]:
									# 	randomRange_R2R1 = parameters.RTT_R2[currentBucketIndex] - parameters.RTT_R1[currentBucketIndex]
									# 	positiveRange_R2R1 = currentRTT - parameters.RTT_R2[currentBucketIndex]
									# 	if random.uniform(0, randomRange_R2R1)*0.5 < positiveRange_R2R1:
									# 		reSelectFlag = True

									if bucketItemCandidate_str in G.nodes[RLLoopNodeCounter]['buckets'][currentBucketIndex] or reSelectFlag:
										bucketItemCandidate_str = worstBucketItemBinary_str[:(currentBucketIndex+1)]
					...
					if RLLoopNodeCounter == observationNode and currentBucketIndex == 0:
						print("Node " + str(RLLoopNodeCounter) + " Epoch score: " + str(currentBucketScore) + " latency: " + str(avgLatency) + " RLCounter 123: " + str(G.nodes[RLLoopNodeCounter]['epochCounters'][0]))
						# f = open("../../../GitHubExperimentsResults/TestResultAug14_2048_RTTR1R2_Simple_DHT.txt", "a")
						# f.write(str(G.nodes[RLLoopNodeCounter]['epochCounters'][0]) + "	" + str(currentBucketScore) + "	" + str(avgLatency) + "\n")
						# f.close()
						print("Node 123 bucket 0: " + str(G.nodes[observationNode]['buckets'][0]))
						if G.nodes[RLLoopNodeCounter]['epochCounters'][0] > 300:
							print("200 Epochs for node 123 bucket 0")
							endTime = time.time()
