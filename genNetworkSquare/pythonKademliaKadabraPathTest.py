...
srcDestPairFile = open('lookupSrcDestPairs.txt', 'r')
srcDestPairLines = srcDestPairFile.readlines()

# stop when there are no remaining src dest pairs
for line in srcDestPairLines:
	srcDestList = line.split()
	# predefined source and destination nodes
	...
	f = open("PathTestRTTRSep17_TwoBlobs.txt", "a")
	...
	f.write(str(lookupResult[2][minLatencyIndex]) + "	" + str(len(lookupResult[2][minLatencyIndex])) + "\n") # + citiesString + "\n")
	# sumNodeLatency = 0
		...
		while len(bucketsToRunRL) > 0:
			...
			while RLLoopNodeCounter < parameters.NETWORK_SIZE:
				if len(G.nodes[RLLoopNodeCounter]['RLTables'][currentBucketIndex][parameters.REINFORCEMENT_LEARNING_TABLE_DEST_COLUMN_INDEX]) >= parameters.EPOCH_QUERIES_NUMBER:
					...
					# 	print("Node " + str(RLLoopNodeCounter) + " Epoch score: " + str(currentBucketScore) + " latency: " + str(avgLatency) + " RLCounter 78: " + str(G.nodes[RLLoopNodeCounter]['epochCounters'][0]))
					...
					# 		print("100 Epochs for node 78 bucket 0")
					# 		endTime = time.time()
