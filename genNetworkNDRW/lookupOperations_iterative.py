...
def simpleLookup(G, src, dest, alpha, useLookupKBR, useLookupDHT, DHTCopiesNumber):
	...
	while alphaCounter < len(currentNodes):
		...
		currentRTT, currentHopCount, currentPathNodes = basicLookup(G, src, currentNodes[alphaCounter], dest, useLookupKBR, useLookupDHT, DHTCopiesNumber, networkEdges[alphaCounter])
		currentRTT += (G[src][currentNodes[alphaCounter]]['weight'] + G.nodes[currentNodes[alphaCounter]]['nodeLatency'])
		currentHopCount += 1
...
def basicLookup(G, realSrc, src, dest, useLookupKBR, useLookupDHT, DHTCopiesNumber, edgesForThisLookup):
	RTT_in_EucDist = -1
	nextHop = None
	currentNode = src
	...
	while not (currentNode in destList):
		...
		for i in G.nodes[currentNode]['buckets'][bucketIndex]:
			...
			if min < 0:
				...
				min = xorResult
				nextHop = i
		thisHopDist = G[bucketOperations.binaryToDecimal(int(nextHop))][realSrc]['weight'] + G.nodes[bucketOperations.binaryToDecimal(int(nextHop))]['nodeLatency'] 
...
def simpleLookupPR(G, src, dest, alpha, useLookupKBR, useLookupDHT, DHTCopiesNumber):
	...
	xorResultsNodes = []
	for i in G.nodes[src]['buckets'][bucketIndex]:
		xorResultsInBucket.append(G[bucketOperations.binaryToDecimal(int(i))][src]['weight']) # PR
		xorResultsNodes.append(i)
	...
	while alphaCounter < len(currentNodes):
		networkEdges[alphaCounter].append((src, currentNodes[alphaCounter]))
		currentRTT, currentHopCount, currentPathNodes = basicLookupPR(G, src, currentNodes[alphaCounter], dest, useLookupKBR, useLookupDHT, DHTCopiesNumber, networkEdges[alphaCounter])
		currentRTT += (G[src][currentNodes[alphaCounter]]['weight'] + G.nodes[currentNodes[alphaCounter]]['nodeLatency'])
		currentHopCount += 1
		RTTs_in_EucDist.append(currentRTT)
		hopCounts.append(currentHopCount)
...
def basicLookupPR(G, realSrc, src, dest, useLookupKBR, useLookupDHT, DHTCopiesNumber, edgesForThisLookup):
	RTT_in_EucDist = -1
	nextHop = None
	currentNode = src
	binCurrentNode = format(currentNode, parameters.NODE_ID_LENGTH_STRING)
	...
	while not (currentNode in destList):
		...
		min = -1
		for i in G.nodes[currentNode]['buckets'][bucketIndex]:
			xorResult = G[bucketOperations.binaryToDecimal(int(i))][currentNode]['weight']
			if min < 0:
				min = xorResult
				nextHop = i
			elif min >= xorResult:
